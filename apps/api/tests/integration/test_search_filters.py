...
def test_radius_filter_returns_nearby_only(client):
    ...
    response = client.get("/igrejas/buscar?lat=-5.7945&lon=-35.2110&radius_km=5&next_hours=1")
...
def test_city_filter_returns_matching_city(client):
    ...
    response = client.get("/igrejas/buscar?city=Natal&radius_km=50&next_hours=1")
    assert response.status_code == 200
    payload = response.json()
